...
if(isset($_POST['submit'])=="Submit Details")
{
    ...
    $accNum = $connection->real_escape_string($_POST['accNum']);
    $exp = $connection->real_escape_string($_POST['exp']);
    $qua = $connection->real_escape_string($_POST['qua']);


    $sql = "UPDATE electrician SET address='$address', city='$city', postcode='$postcode', u_state='$state', hp='$hp', qualification='$qua', exper_duration='$exp', email='$email', accountNum='$accNum' where electricianID ='$User'";
...
<div class="PageContainer">
    ...
    <?php
    $query = "SELECT * FROM electrician where id ='$User'";
        ...
        while($row = mysqli_fetch_array($result))
        {
            if($row['id']== $User)
            {
                    ...
                    <div class="overlay_form">
                        <form method="post" action="Personal.php">
                            ...
                            <div class="overlay-inner">
                                ...
                                <div>
                                    <input style="width: 100%;" class="box" type="text" name="exp" id="exp" value="<?php echo $row['exper_duration'] ?>" placeholder="EXPERIENCE">
                                </div>

                                <br>
                                <label for="qua">Qualification: </label>
                                <div>
                                    <input style="width: 100%;" class="box" type="text" name="qua" id="qua" value="<?php echo $row['qualification'] ?>" placeholder="QUALIFICATIONS">
                                </div>
                ...
                <div class="rowCard1">
                    ...
                    <input style="float: right" onclick="on('<?php echo $row["email"]; ?>','<?php echo $row["hp"];?>','<?php echo $row["address"];?>','<?php echo $row["postcode"];?>','<?php echo $row["city"];?>','<?php echo $row["u_state"]; ?>','<?php echo $row["exper_duration"];?>','<?php echo "qualification";?>')"  class="button"  type="submit" name="submit" value="Edit My Details">
                </div>


                <div class="rowPersonal">

                    <div class="column1Personal">
                    ...
                    <div class="column2Personal">
                        <div class="cardPersonal">
                            <h2><i class="fa fa-suitcase fa-fw headingTitle "></i>Work Experience</h2>
                            <p><?php echo $row['exper_duration']?></p>
                            <h2><i class="fa fa-certificate fa-fw headingTitle"></i>Education</h2>
...
<script>
    ...
    function on(email,hp,add,postcode,city,state,exp,qua) {
