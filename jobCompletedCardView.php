...
if(mysqli_num_rows($resultR) > 0)
{
    while($rowR = mysqli_fetch_array($resultR))
    {
        if($rowR ['JobStatus'] == "Completed" && $rowR ['PaymentStatus'] == "Pending")
        {
            ...
            <div class="cardView" style="background-color: tomato ; color: black">
                <div class="containerView" >
                    ...
                    <input onclick="on('<?php echo $row["idJob"]; ?>','<?php echo $row["TaskName"]; ?>','<?php echo $row["Date"];?>',
                            '<?php echo $row["Time"];?>','<?php echo $row["Address"];?>','<?php echo $row["Description"];?>',
                            '<?php echo $row["state"];?>','<?php echo $row["EmployerID"];?>','<?php echo "-";?>','<?php echo "-";?>')"
                           class="button button4"  type="submit" name="submit" value="ViewDetails">
                </div>
            </div>
        ...
        elseif($rowR ['JobStatus'] == "Completed" && $rowR ['PaymentStatus'] == "Completed")
        {
            //job completed fully
            ?>
            ...
            <div class="cardView">
                <div class="containerView">
                    <form method="post" action="jobCompleted2.php">
                        ...
                        <input type="hidden" name="hidden_id" value="<?php echo $row["idJob"]; ?>">
                        <input type="hidden" name="hidden_empid" value="<?php echo $row["EmployerID"]; ?>">
                        <input type="hidden" name="hidden_review" value="<?php echo $rowR['review']; ?>">
                        <input type="hidden" name="hidden_rating" value="<?php echo $rowR['rating']; ?>">
                    </form>
                    <input onclick="on('<?php echo $row["idJob"]; ?>','<?php echo $row["TaskName"]; ?>','<?php echo $row["Date"];?>',
                            '<?php echo $row["Time"];?>','<?php echo $row["Address"];?>','<?php echo $row["Description"];?>',
                            '<?php echo $row["state"];?>','<?php echo $row["EmployerID"];?>','<?php echo "-";?>','<?php echo "-";?>')"
